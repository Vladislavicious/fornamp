...
class WindowAdd(ctk.CTkToplevel):
    ...
    def panel_add(self):    #панель добавления шагов, продуктов, заказа
        ...
        self.title_name_label = ctk.CTkLabel(self.topbar, text="Добавление заказа", fg_color="transparent",
    ...
    def add_order_field(self):  #поля ввода для создания заказа и конпка выхода в главное меню
        ...
        self.entry_data_order = ctk.CTkEntry(frame_order_field, placeholder_text="ДД-ММ-ГГГГ")
    ...
    def check_order_field(self):
        ...
        date_text = self.entry_data_order.get()
        if len(date_text) != 10:
            ...
            if date is not None:
                ...
                self.entry_data_order.configure(fg_color="#f9f9fa", border_color="#61bf0d",
                                                placeholder_text="", text=date.strftime("%d-%m-%Y"))
            else:
                self.edit_data_vidachi_field()
                check = False
    ...
    def add_new_order(self):
        if self.check_order_field() is True:
            dat = bh_order.date.today()
            order = bh_order.Order(1, self.main_window.user.login, dat, self.dat_of_vidacha,
                                   self.list_product, self.entry_commentariy_order.get())      # это затычка надо поменять принимаемые данные
